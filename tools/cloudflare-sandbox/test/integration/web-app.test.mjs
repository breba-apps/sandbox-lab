...
import { connectWs, uniqueSession, wsBaseUrlFromEnv } from '../helpers/ws-client.mjs';

const TEST_TIMEOUT_MS = 120_000;
const clients = new Set();

function httpBaseUrl() {
  const url = new URL(wsBaseUrlFromEnv());
...
test(
  'agent sandbox startup endpoint prepares a sandbox without WebSocket bridge',
  { timeout: TEST_TIMEOUT_MS },
  async () => {
    const session = uniqueSession('local-start');
    const startUrl = `${httpBaseUrl()}/sandbox/${session}/start`;

    const getResp = await fetch(startUrl);
    assert.equal(getResp.status, 405);
    assert.equal(getResp.headers.get('allow'), 'POST');

    const postResp = await fetch(startUrl, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ reset: true })
    });
    assert.equal(postResp.status, 200);
    assert.deepEqual(await postResp.json(), { ok: true, session });
  }
);
